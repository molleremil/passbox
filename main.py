...
import pyperclip
import json
from pass_check import pwned_api_check
...
def check_if_leaked(password):
    count = pwned_api_check(password)
    if count:
        messagebox.showwarning(
            title="Warning!", message=f"'{password}' was found {count} times... change your password now!")
    else:
        messagebox.showinfo(
            title="Good News!", message=f"'{password}' was NOT found. You're safe!")
...
check_if_leaked_button = Button(text="Check If Leaked", width=36, fg="#088AF6",
                                command=lambda: check_if_leaked(password_input.get()))
check_if_leaked_button.grid(row=5, column=1, columnspan=2)
